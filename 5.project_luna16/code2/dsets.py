...
raw_cache = getCache('pytorchproject_raw')
...
def getCandidateInfoList(requireOnDisk_bool=True):
    ...
    # the subsets yet.
    mhd_list = glob.glob('data/lunadata/subset*/*.mhd')
    presentOnDisk_set = {os.path.split(p)[-1][:-4] for p in mhd_list}

    diameter_dict = {}
    with open('data/lunadata/annotations.csv', "r") as f:
    ...
    candidateInfo_list = []
    with open('data/lunadata/candidates.csv', "r") as f:
        for row in list(csv.reader(f))[1:]:
            series_uid = row[0]
...
class Ct:
    def __init__(self, series_uid):
        mhd_path = glob.glob(
            'data/lunadata/subset*/{}.mhd'.format(series_uid)
        )[0]
